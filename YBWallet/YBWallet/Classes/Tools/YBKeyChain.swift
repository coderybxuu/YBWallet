...
public final class YBKeychain {
    ...
    public func accessibility(_ accessibility: Accessibility) -> YBKeychain {
        var options = self.options
        options.accessibility = accessibility
        return YBKeychain(options)
    }
    
    @available(iOS 8.0, OSX 10.10, *)
    @available(watchOS, unavailable)
    public func accessibility(_ accessibility: Accessibility, authenticationPolicy: AuthenticationPolicy) -> YBKeychain {
        var options = self.options
        options.accessibility = accessibility
    ...
    public func synchronizable(_ synchronizable: Bool) -> YBKeychain {
        var options = self.options
        options.synchronizable = synchronizable
        return YBKeychain(options)
    }
    
    public func label(_ label: String) -> YBKeychain {
        var options = self.options
        options.label = label
        return YBKeychain(options)
    }
    
    public func comment(_ comment: String) -> YBKeychain {
        var options = self.options
        options.comment = comment
        return YBKeychain(options)
    }
    
    public func attributes(_ attributes: [String: Any]) -> YBKeychain {
        var options = self.options
    ...
    @available(iOS 8.0, OSX 10.10, *)
    @available(watchOS, unavailable)
    public func authenticationPrompt(_ authenticationPrompt: String) -> YBKeychain {
        var options = self.options
    ...
    #if os(iOS) || os(OSX)
    @available(iOS 9.0, OSX 10.11, *)
    public func authenticationContext(_ authenticationContext: LAContext) -> YBKeychain {
        var options = self.options
...
extension YBKeychain: CustomStringConvertible, CustomDebugStringConvertible {
